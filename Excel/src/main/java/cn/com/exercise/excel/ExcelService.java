...
public class ExcelService {
    ...
    public Integer uploadExcel(MultipartFile file,String uploadPath){
        ...
        String excelPath = uploadPath + name + "_" + date + "_" + type;
    ...
    private Integer excelHandleWorker(String excelPath) {
        ...
        try {
            ...
            System.out.println("行数："+rowLen);
            //更新 参数
            Integer[] intIdsArray = new Integer[rowLen];
            for (int i = 1; i <= rowLen+1; i++) {
                ...
                if(cell.getCellType()==Cell.CELL_TYPE_NUMERIC){
                    ...
                    id = numericCellValue.toString();
                }else if(cell.getCellType()==Cell.CELL_TYPE_STRING) {
                    id = cell.getStringCellValue();
                }else if(cell.getCellType()==Cell.CELL_TYPE_BLANK){
                    continue;
                }
                intIdsArray[i-1]= Integer.parseInt(id);
                result++;
            }
            //入库
//            excelMapper.upload(intIdsArray);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
